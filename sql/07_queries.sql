...
SELECT 
    ...
    ROUND(
        (SUM(p.total_amount) - LAG(SUM(p.total_amount)) OVER (PARTITION BY s.store_id ORDER BY DATE_FORMAT(p.purchase_date, '%Y-%m'))) 
        / LAG(SUM(p.total_amount)) OVER (PARTITION BY s.store_id ORDER BY DATE_FORMAT(p.purchase_date, '%Y-%m')) * 100,
    2) AS mom_growth_pct
FROM Store s
...
GROUP BY s.store_id, s.store_name, report_month
ORDER BY s.store_name, report_month DESC;
...
WITH CustomerFinancials AS (
    SELECT 
        ...
        (SUM(p.total_amount) - IFNULL(SUM(r.refund_amount), 0)) AS net_value
    FROM Customer c
...
SELECT 
    ...
    total_returns,
    net_value,
    RANK() OVER (PARTITION BY join_cohort ORDER BY net_value DESC) AS cohort_rank
FROM CustomerFinancials
ORDER BY join_cohort DESC, cohort_rank;
...
SELECT 
    ...
    ROUND(COUNT(DISTINCT ri.return_item_id) / COUNT(DISTINCT pi.purchase_item_id) * 100, 1) AS return_rate_pct,
    -- Composite Score: (Revenue/1000) + (Rating * 20) - (Return Rate * 5)
    ROUND(
        (SUM(pi.line_total) / 1000) + 
        (IFNULL(AVG(rev.rating), 3) * 20) - 
        ((COUNT(DISTINCT ri.return_item_id) / COUNT(DISTINCT pi.purchase_item_id) * 100) * 5), 
    0) AS quality_score
FROM Vendor v
...
SELECT 
    ...
    ROUND(SUM(CASE WHEN p.customer_id IS NOT NULL THEN p.total_amount ELSE 0 END) / SUM(p.total_amount) * 100, 1) AS loyalty_sales_pct,
    -- Percentile Rank compared to all employees
    ROUND(PERCENT_RANK() OVER (ORDER BY SUM(p.total_amount)), 2) AS revenue_percentile
FROM Employee e
...
GROUP BY e.employee_id, e.first_name, e.last_name, s.store_name
...
WITH SalesMix AS (
    ...
    JOIN PurchaseItem pi ON i.inventory_id = pi.inventory_id
    JOIN Purchase pu ON pi.purchase_id = pu.purchase_id
    GROUP BY p.platform, p.esrb_rating, period
...
SELECT 
    ...
    CONCAT(ROUND(((s2.revenue - s1.revenue) / s1.revenue * 100), 1), '%') AS growth_pct
FROM SalesMix s1
JOIN SalesMix s2 
    ON s1.platform = s2.platform 
    AND s1.esrb_rating = s2.esrb_rating
WHERE s1.period = 'Period_1' AND s2.period = 'Period_2'
...
SELECT 
    ...
    (COUNT(DISTINCT ri.return_item_id) / COUNT(DISTINCT pi.purchase_item_id)) * 100 AS return_rate_pct
FROM Product p
JOIN Inventory i ON p.product_id = i.product_id
JOIN PurchaseItem pi ON i.inventory_id = pi.inventory_id
LEFT JOIN Review rev ON p.product_id = rev.product_id
LEFT JOIN ReturnItem ri ON pi.purchase_item_id = ri.purchase_item_id
GROUP BY p.product_id, p.name, p.platform
HAVING 
    -- Scenario A: Good Rating (>4) but High Returns (>15%)
    (avg_rating >= 4.0 AND return_rate_pct > 15)
    OR 
    -- Scenario B: Bad Rating (<3) but Low Returns (<5%) - potentially suspicious
    (avg_rating <= 3.0 AND return_rate_pct < 5)
...
SELECT 
    ...
    FLOOR((inv_source.quantity_available - inv_source.restock_threshold) / 2) AS transfer_suggestion
...
WHERE 
    -- Source is Overstocked (> 2x Threshold)
    inv_source.quantity_available > (inv_source.restock_threshold * 2)
    -- Destination is Understocked (Below Threshold)
...
SELECT 
    ...
    DATEDIFF(
        p.purchase_date, 
        LAG(p.purchase_date) OVER (PARTITION BY c.customer_id ORDER BY p.purchase_date)
...
WITH ProductMargins AS (
    SELECT 
        ...
        AVG((i.current_price - i.purchase_price) / i.current_price) * 100 AS margin_pct
    FROM Product p
...
RankedSales AS (
    SELECT 
        *,
        ...
        NTILE(4) OVER (PARTITION BY category_name ORDER BY total_sold DESC) AS sales_quartile
    FROM ProductMargins
)
SELECT 
    category_name,
...
FROM RankedSales
WHERE 
    sales_quartile = 1 -- Top 25% by volume
